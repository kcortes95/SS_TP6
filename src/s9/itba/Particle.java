...
public class Particle {
	...
	public static final double dVel = 2.0;
	...
	public Particle(double rx, double ry, double vx, double vy, double ax,
			double ay, double radius, double mass, Color color) {
		this.vx = vx;
		this.vy = vy;
		this.ax = ax;
		this.ay = ay;
	...
	public Particle(int ID, double rx, double ry, double vx, double vy,
	...
	public double getDistance(Particle other) {
		return Math.sqrt(Math.pow(this.rx - other.rx, 2)
	...
	public double getSuperposition(Particle p) {
		return this.r
				+ p.r
				- Math.sqrt(Math.pow(this.rx - p.rx, 2)
						+ Math.pow(this.ry - p.ry, 2));
	}
	...
	private double calculateTanForce(Particle p, double kt) {
		Vector tVersor = getTanVersor(p);
		return -kt * getSuperposition(p)
				* (getTanVel(tVersor) - p.getTanVel(tVersor));
	}
	...
	private void calculateDAngle(Vector[] target) {
		Vector obj = new Vector((target[0].x + target[1].x) / 2,
				(target[0].y + target[1].y) / 2); // por ahora va al medio
													// despues vemos
